...
def generate_random_files(num_files, min_size, max_size):
    dataset_name = valohai.parameters("dataset-name").value
    dataset_version = valohai.parameters("dataset-version").value
    for i in range(1, num_files + 1):
        ...
        with open(output, "wb") as f:
            f.write(content)
        metadata = {
            "valohai.dataset-versions": [f"dataset://{dataset_name}/{dataset_version}"]
        }
